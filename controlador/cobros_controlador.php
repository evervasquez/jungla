...
//                print_r($this->_vista->datos_ventas);exit;
    ...
    public function cronograma($idventa, $monto_restante){
        ...
        $this->_vista->btn_action = BASE_URL.'cobros/amortizar/'.$idventa.'/'.$monto_restante;
                ...
                if($datos_cuota_cobro[$i]['MONTO_CUOTA']>$datos_cuota_cobro[$i]['MONTO_COBRADO']){
                    ...
                    if($monto_amortizado!=0){
                        if($monto_restantexcuota>=$monto_amortizado){
                            ...
                            $this->_amortizacion_cobro->idmovimiento_caja=$dato_movimiento_caja[0]['X_IDMOVIMIENTO_CAJA'];
